...
import type { DimensionOption } from "@/features/dimensions/dimension-select";
...
/**
 * One row of tappable chips, one per option, with an explicit "everything"
 * chip in front. This is the filter for both dimensions, on the dashboard and
 ...
 * scroll a list and close it again to make one choice. The chips replace a
 * `DimensionSelect` on the filter rows; the transaction form keeps the select,
 * because a required choice with no "everything" answer is a different
 * control.
 *
 * Behaviour carried over from the select it replaces, each paid for by a bug:
 *
 ...
 * movement between chips and the pressed state for free, and "exactly one
...
export function DimensionChips({
  ...
  /** The selected option's id, or `null` for everything. */
  value: string | null;
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <ToggleGroup
        ...
        className="flex-wrap"
      >
        <Chip value={EVERYTHING}>{copy.everything}</Chip>
        {options.map((option) => (
        ...
        {isStale && (
          // Pressed, because it *is* the active filter. Tapping it clears.
